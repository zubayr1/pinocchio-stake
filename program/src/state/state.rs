...
use crate::{
    error::StakeError,
    helpers::{StakeHistoryEntry, StakeHistoryGetEntry},
};
...
/*
    StakeState = "Please use `StakeStateV2` instead, and match the third `StakeFlags` field when matching `StakeStateV2::Stake` to resolve any breakage. For example, `if let StakeState::Stake(meta, stake)` becomes `if let StakeStateV2::Stake(meta, stake, _stake_flags)`."
*/
...
impl Authorized {
    ...
    //instead of accepting an HashSet, the check accepts a &[Pubkey]
    pub fn check(
        &self,
        signers: &[Pubkey],
